...
export class EditarPetComponent implements OnInit {
  ...
  pushNovodietaExistente(dieta: Dieta){
    const dietaForm = this.builder.group({
      horarioAlimentacao: new FormControl<Date>(dieta.horarioAlimentacao,Validators.required),
      quantidade :  new FormControl<number>(dieta.quantidade,Validators.required),
      observacoes :  new FormControl<string>(dieta.observacoes),
      // alimentos que não estão na dieta
      alimentosDisponiveis: new FormControl<Alimento[]>(this.alimentos.filter(al => dieta.alimentos.indexOf(al) != -1)),
      alimentos:  new FormControl<Alimento[]>([],Validators.required)
    });
    this.dieta.push(dietaForm);
  }
